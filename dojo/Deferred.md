...
resolve(value, strict)           |Called by the asnyc process, when complete, providing any results to be passed to the callback.  The second argument, which is optional, if ``true`` means it will throw an error if the promise is fulfilled.  Defaults to ``false``.
